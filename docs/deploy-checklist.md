...
| Port | `587` |
| Username | `resend`（固定文字列。メールアドレスではない） |
| Password | Resend APIキー（`supabase-case8` / Sending access のみ） |

#### 認証結果（Gmail の「原文を表示」で確認）
...
DKIM の署名ドメインが From と一致しており、DMARC まで通っている。

#### 送信レート上限は 30 emails/h のまま変更しない

読み手7名が全員パスワード再設定を行っても7通で、再試行を含めても余裕がある。
**この上限は、不具合や攻撃でメール送信がループしたときの被害を止める安全弁**として働く。
必要がないのに緩めれば、その保護を捨てることになる。

#### 到達確認の結果

| | 1回目（既定テンプレート） | 2回目（日本語テンプレート） |
|---|---|---|
| 件名 | `Reset your password` | `【LUMINA 売上分析ダッシュボード】パスワード再設定のご案内` |
| 本文 | 英語 | 日本語 |
| 配置 | **迷惑メール** | **受信トレイ** |

リンクをクリックすると本番の `/auth/reset-password` が開き、
対象アカウントが表示されるところまで確認した（トークン交換が成功している）。

**ただし2回目の結果は、テンプレート日本語化の効果だけではない。**
1回目を「迷惑メールでないことを報告」した後に送っているため、
Gmail 側の学習も同時に効いている。**日本語化したから受信トレイに入った、とは断定できない。**

#### 迷惑メール判定について（正直な限界）

1回目が迷惑メールに入った理由として Gmail は
「以前迷惑メールと判断されたメールに類似しています」と表示した。
認証（SPF/DKIM/DMARC）はすべて PASS しているため、原因は**内容と送信実績**である。

**送信実績のないドメインは、最初のうち迷惑メールに入りやすい。**
設定を直せば必ず受信トレイに入る、とは言えない。
今回できたのは「認証を正しく通し、内容を怪しくない日本語にする」ところまで。

実案件では**クライアントの既存ドメイン**（すでに送信実績がある）を使うため、
この問題は起きにくい。
